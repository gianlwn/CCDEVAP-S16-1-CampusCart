...
- All user-controlled text (listing titles/descriptions, bios, reviews, report reasons, etc.) is HTML-escaped at render time (`escapeHtml()` in `components.js`/`adminComponents.js`) before being written via `innerHTML` in cards, detail modals, and the notification list. Confirm-dialog prompts (`showConfirm()` in `components.js`) are set via `textContent`, so they're always rendered as literal text no matter what they contain. Toast messages (`toastNotification.js`) are the one exception: the title is set via `textContent`, but the message body is still written via `innerHTML` — callers are responsible for calling `escapeHtml()` on any user-supplied value before passing it in, which every call site does. Together these close a stored-XSS hole that affected every page rendering API data, including the admin panel (e.g. approving a listing titled `<img src=x onerror=...>` used to execute in the success toast).
