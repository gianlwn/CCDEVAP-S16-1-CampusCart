...
function buildNewListing(name, price, category, condition, description, quantity, location) {
  return {
    id: Date.now(),
    name,
...
function saveListingEdit(listings, id, name, price, category, condition, location, description, quantity) {
  if (!listings.find(l => l.id === id)) return { success: false, error: 'not_found' };
