function applyItemReview(items, id, rating, comment) {
  return items.map(item =>
    item.id === id ? { ...item, userRating: rating, userComment: comment } : item
  );
}

function updateRatingEntry(ratings, id, star, review) {
  return ratings.map(r =>
    r.id === id ? { ...r, rating: star, review } : r
  );
}
...
function submitItemReview(items, id, rating, comment) {
  ...
  if (!items.find(c => c.id === id)) return { success: false, error: 'not_found' };
...
function editRatingEntry(ratings, id, star, review) {
  ...
  return {
    success: true,
    ratings: updateRatingEntry(ratings, id, star || item.rating, review || item.review),
  };
}
...
function claimCartItem(items, id) {
  ...
  if (!item) return { success: false, error: 'not_found' };
  return { success: true, itemName: item.name, items: removeCartItemById(items, id) };
}
