...
function getAdmins() { return _adminsData; }
...
function editAdminRecord(oldName, newName, email) {
  ...
  if (admin) { admin.username = newName; admin.email = email; }
}
...
function getUsers() { return _usersData; }
...
function getListingApprovals() { return _listingApprovalsData; }
...
function getCategories() { return _categoriesData; }
...
function getReports() { return _reportsData; }
...
function revokeAdmin(name) {
  ...
  if (admin.status === 'inactive') return { success: false, error: 'already_revoked' };
