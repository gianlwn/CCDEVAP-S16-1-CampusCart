...
async function handleLogin() {
  ...
  if (!email.toLowerCase().endsWith(".edu.ph")) {
    showToast(
      "Invalid Email",
      "Only valid .edu.ph addresses are accepted.",
      "error",
    );
    return;
  }
...
async function handleConfirm(event) {
  ...
  if (!codeSent) {
    showToast(
      "Send Code First",
      "Please send a verification code before confirming.",
      "warning",
    );
    return;
  }
  ...
  try {
    ...
    showToast(
      "Verified!",
      "Email confirmed. Redirecting to registration…",
      "success",
      1500,
    );
    setTimeout(() => {
...
async function handleSendRecoveryCode() {
  ...
  if (!email || !email.toLowerCase().endsWith(".edu.ph")) {
    showToast(
      "Invalid Email",
      "Please enter a valid .edu.ph address.",
      "error",
    );
    return;
  }
...
async function handleConfirmRecoveryCode() {
  ...
  if (!recCodeSent) {
    showToast(
      "Send Code First",
      "Please request a recovery code first.",
      "warning",
    );
    return;
  }
